...
function StatusBadge({ active }: { active: boolean }) {
  return (
    <span className={`inline-flex items-center rounded-md border backdrop-blur-md px-2.5 py-1 text-[11px] font-bold uppercase tracking-widest shadow-[var(--shadow-sm)] ${
      active 
        ? "border-[var(--color-info)]/30 bg-[var(--surface-3)]/80 text-[var(--color-info)]"
        : "border-[var(--color-pending)]/30 bg-[var(--surface-3)]/80 text-[var(--color-pending)]"
    }`}>
      ...
      {viewMode === "list" ? (
        <div className="flex flex-col gap-3">
          {services.map((service) => (
            <article
              key={service.id}
              className="group flex flex-col sm:flex-row sm:items-center justify-between gap-4 rounded-[var(--radius-lg)] border border-[var(--border-strong)] bg-[var(--surface-3)] p-4 shadow-[var(--shadow-sm)] transition-shadow hover:shadow-[var(--shadow-md)]"
            >
        ...
        <div className="grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
          {services.map((service) => (
            <article
              key={service.id}
              className="group flex flex-col overflow-hidden rounded-[var(--radius-lg)] border border-[var(--border-strong)] bg-[var(--surface-3)] shadow-[var(--shadow-sm)] transition-shadow hover:shadow-[var(--shadow-md)]"
            >
              ...
              <div className="border-t border-[var(--border-strong)] bg-[var(--surface-2)] p-3 flex justify-end">
                <ServiceActionsMenu
                  service={service}
                  disabled={disabled}
                  onEdit={onEdit}
