...
export default function BrandLogo({
  ...
  const softDropShadowId = `softDropShadow_${uid}`;
  const accentShadowId = `accentShadow_${uid}`;
  const ringShadowId = `ringShadow_${uid}`;
  const innerGridShadowId = `innerGridShadow_${uid}`;
  const bgGradId = `bgGrad_${uid}`;
  const calendarSurfaceId = `calendarSurface_${uid}`;
  const gridSurfaceId = `gridSurface_${uid}`;
  const activeBrandGradId = `activeBrandGrad_${uid}`;
  const chromeCylinderId = `chromeCylinder_${uid}`;
  const ringBaseId = `ringBase_${uid}`;

  return (
    <div className={cn("inline-flex items-center gap-2 select-none", containerClassName)}>
      {/* 
        Icono SVG Premium Oficial de Agenda Web
      */}
      <svg
        width={size}
      ...
      >
        <defs>
          ...
          <filter id={softDropShadowId} x="-20%" y="-20%" width="140%" height="140%">
          ...
          <filter id={accentShadowId} x="-20%" y="-20%" width="140%" height="140%">
            <feDropShadow dx="6" dy="14" stdDeviation="14" floodColor="#e03000" floodOpacity="0.35" />
          </filter>

          <filter id={ringShadowId} x="-20%" y="-20%" width="140%" height="140%">
            <feDropShadow dx="0" dy="8" stdDeviation="8" floodColor="#000000" floodOpacity="0.3" />
          </filter>

          <filter id={innerGridShadowId} x="-10%" y="-10%" width="120%" height="120%">
          ...
          <linearGradient id={calendarSurfaceId} x1="0%" y1="0%" x2="0%" y2="100%">
            <stop offset="0%" stopColor="#ECEEF3" />
            <stop offset="100%" stopColor="#E0E2E8" />
          </linearGradient>

          <linearGradient id={gridSurfaceId} x1="0%" y1="0%" x2="0%" y2="100%">
            <stop offset="0%" stopColor="#FFFFFF" />
            <stop offset="100%" stopColor="#F8F8FB" />
          </linearGradient>

          {/* Degradado Activo de la Marca (Naranja → Rojo) */}
          <linearGradient id={activeBrandGradId} x1="0%" y1="0%" x2="100%" y2="100%">
          ...
          <linearGradient id={chromeCylinderId} x1="0%" y1="0%" x2="100%" y2="0%">
          ...
          <linearGradient id={ringBaseId} x1="0%" y1="0%" x2="0%" y2="100%">
        ...
        {/* Fondo opcional */}
        {showBg && <rect x="70" y="50" width="372" height="412" rx="84" fill={`url(#${bgGradId})`} />}

        {/* Calendario Base */}
        <rect x="76" y="96" width="360" height="360" rx="52" fill={`url(#${calendarSurfaceId})`} />
        <rect x="76" y="96" width="360" height="360" rx="52" fill="none" stroke="#CBCDD4" strokeWidth="2.5" />

        {/* Cuadros Internos Estáticos */}
        <rect x="116" y="136" width="132" height="132" rx="32" fill={`url(#${gridSurfaceId})`} filter={`url(#${innerGridShadowId})`} />
        <rect x="116" y="136" width="132" height="132" rx="32" fill="none" stroke="#D0D2D9" strokeWidth="1.5" />

        <rect x="264" y="136" width="132" height="132" rx="32" fill={`url(#${gridSurfaceId})`} filter={`url(#${innerGridShadowId})`} />
        <rect x="264" y="136" width="132" height="132" rx="32" fill="none" stroke="#D0D2D9" strokeWidth="1.5" />

        <rect x="116" y="284" width="132" height="132" rx="32" fill={`url(#${gridSurfaceId})`} filter={`url(#${innerGridShadowId})`} />
        ...
        <rect
          ...
          height="132"
          rx="32"
          fill={`url(#${activeBrandGradId})`}
          filter={`url(#${accentShadowId})`}
        />
        ...
        {/* CLIPS METÁLICOS (Efecto Cilíndrico y Orificios de Papel) */}

        {/* Orificio Izquierdo */}
        <ellipse cx="166" cy="124" rx="18" ry="12" fill="#D8DAE0" />
        <ellipse cx="166" cy="126" rx="15" ry="9" fill={`url(#${ringBaseId})`} />

        {/* Orificio Derecho */}
        <ellipse cx="346" cy="124" rx="18" ry="12" fill="#D8DAE0" />
        <ellipse cx="346" cy="126" rx="15" ry="9" fill={`url(#${ringBaseId})`} />

        {/* Clip Cilíndrico Izquierdo */}
        <rect x="154" y="60" width="24" height="68" rx="12" fill={`url(#${chromeCylinderId})`} />
        ...
        <rect x="334" y="60" width="24" height="68" rx="12" fill={`url(#${chromeCylinderId})`} />
      ...
      {variant === "full" && (
        <div className="flex items-baseline leading-none font-sans text-[1.22rem] tracking-tight">
          ...
          <span className="font-bold text-[var(--app-primary)] ml-0.5">
            Web
          </span>
        </div>
      )}
